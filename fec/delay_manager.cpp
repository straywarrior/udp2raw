...
ev_timer &delay_manager_t::get_timer() {
    return timer;
}
...
int delay_manager_t::add(my_time_t delay, fec_send_cb cb, void *ctx, char *data, int len) {
    ...
    if (loop == 0) {
        mylog(log_warn, "delay_manager has no loop, send immediately\n");
        delay_data.data = data;
        int ret = delay_data.handle();
        if (ret != 0) {
            mylog(log_trace, "handle() return %d\n", ret);
